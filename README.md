...
docker run -d -v /tmp/.X11-unix:/tmp/.X11-unix:ro -e DISPLAY \
  braintwister/ubuntu-18.04-cmake-3.13-gcc-7-conan-1.9-docker-18.09-eclipse-cpp-2018-09
```
...
services:
  ...
  eclipse:
    image: braintwister/ubuntu-18.04-cmake-3.13-gcc-7-conan-1.9-docker-18.09-eclipse-cpp-2018-09
    volumes:
...
services:
  ...
  eclipse:
    image: braintwister/ubuntu-18.04-cuda-9.2-cmake-3.13-gcc-7-conan-1.9-nsight
    runtime: nvidia
    volumes:
      - /tmp/.X11-unix:/tmp/.X11-unix:ro
    environment:
...
```bash
docker run -d -v /tmp/.X11-unix:/tmp/.X11-unix:ro -e DISPLAY \
  braintwister/ubuntu-18.04-cmake-3.13-clang-7-conan-1.9-docker-18.09-vscode-1.29
```
...
pipeline {
  ...
  agent {
    docker {
      image 'braintwister/ubuntu-18.04-cmake-3.13-clang-7-conan-1.9'
    }
  }
