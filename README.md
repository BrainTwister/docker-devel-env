...
docker run -d -v /tmp/.X11-unix:/tmp/.X11-unix:ro -e DISPLAY --privileged \
  braintwister/ubuntu-18.04-cmake-3.14-gcc-9-conan-1.17-docker-18.09-eclipse-cpp-2019-06
```
...
services:
  ...
  eclipse:
    image: braintwister/ubuntu-18.04-cmake-3.14-gcc-9-conan-1.17-docker-18.09-eclipse-cpp-2019-06
    volumes:
...
services:
  ...
  eclipse:
    image: braintwister/ubuntu-18.04-cuda-10.1-cmake-3.14-gcc-9-conan-1.17-nsight
    runtime: nvidia
    volumes:
      - /tmp/.X11-unix:/tmp/.X11-unix:ro
    environment:
...
```bash
docker run -d -v /tmp/.X11-unix:/tmp/.X11-unix:ro -e DISPLAY --privileged \
  braintwister/ubuntu-18.04-cmake-3.14-clang-7-conan-1.17-docker-18.09-vscode-1.32
```
...
pipeline {
  ...
  agent {
    docker {
      image 'braintwister/ubuntu-18.04-cmake-3.14-clang-7-conan-1.17'
    }
  }
