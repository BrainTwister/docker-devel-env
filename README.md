...
```bash
docker run -d -v /tmp/.X11-unix:/tmp/.X11-unix:ro -e DISPLAY --privileged \
  braintwister/ubuntu-20.04-clang-11-vscode-1.53.2
```
...
services:
  ...
  vscode:
    image: braintwister/ubuntu-20.04-clang-11-vscode-1.53.2
    volumes:
      - /tmp/.X11-unix:/tmp/.X11-unix:ro 
      - home:/home/${USER_NAME}
...
docker run -d --runtime=nvidia -e DISPLAY \
  braintwister/ubuntu-20.04-cuda-devel-11.0-tensorflow-gpu-2.0-vscode-1.53.2
```
